exports.up = async function up(knex) {
  ...
  await knex.schema.createTable("shipping_orders", (table) => {
    ...
    table.check("status in ('QUOTED', 'APPROVED')", [], "shipping_orders_status_check");
  ...
  await knex.schema.createTable("shipping_order_items", (table) => {
    ...
    table.check("total_amount >= 0", [], "shipping_order_items_total_amount_check");
  });
};
...
exports.down = async function down(knex) {
  ...
  await knex.schema.raw("alter table products drop constraint products_reserved_stock_check");
  await knex.schema.alterTable("products", (table) => {
