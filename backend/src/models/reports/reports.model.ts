...
export async function getReportsOverview(): Promise<ReportsOverview> {
  const [salesSummary, lowStockProducts, openShippingOrders, openPickupReservations, openCashRegister] =
    await Promise.all([
      db("sales")
        .select<SalesSummaryRow[]>([
          db.raw("count(*)::text as count"),
          db.raw("coalesce(sum(total_amount), 0)::numeric(12, 2)::text as \"totalAmount\""),
        ])
        .first(),
      db("products")
        .where("active", true)
